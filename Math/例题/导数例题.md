...
$eg11.
\newline y=f(x)由方程y-x=e^{x(1-y)}确定,\lim\limits_{n\to \infty}(f(\frac{1}{n})-1)=?$

$eg12.
\newline x=0处不可导$

$eg13.
\newline y=\sin [f(x^2)],f(x)有二阶导数,求\frac{dy^2}{d^2x}=?$

$eg14.
\newline y=y(x)是由方程x^2-y+1=e^y确定隐函数，则\frac{dy^2}{d^2x}=?$

$eg15.
\newline \begin{cases}
y=t\sin t+\cos t
\cr x=\sin t
\end{cases}\quad (t为参数),\frac{dy^2}{d^2x}|_{t=\frac{\pi}{4}}=?$
